...
function home() {
 ...
 <div class="home-layout"><div class="home-primary"><section class="home-section intro-section"><div class="section-label"><h2>关于我</h2><span>ABOUT</span></div><div class="section-content"><p class="intro-text">电子科技大学在读，关注 Agent 的执行流程、上下文管理与可验证交付。通过真实使用中的问题，练习把模型能力变成可靠、顺手的产品。</p><div class="inline-links"><a href="${url('about')}">更多介绍 <span aria-hidden="true">→</span></a><a href="${config.githubUrl}" target="_blank" rel="noopener noreferrer">GitHub ↗</a></div></div></section>
 <section class="home-section"><div class="section-label"><h2>项目</h2><span>SELECTED WORK</span></div><div class="section-content"><div class="compact-repos">${config.repositories.slice(0,4).map(projectRow).join('')}</div><a class="more-link" href="${url('projects')}">查看全部 ${config.repositories.length} 个项目 <span aria-hidden="true">→</span></a></div></section>
 <section class="home-section"><div class="section-label"><h2>文章</h2><span>WRITING</span></div><div class="section-content">${posts.map(essayRow).join('')}<a class="more-link" href="${url('articles')}">浏览文章目录 <span aria-hidden="true">→</span></a></div></section></div>
 <aside class="home-rail"><section class="rail-block"><span class="eyebrow">OPEN TO INTERNSHIPS</span><h2>一起做有用的产品。</h2><p>可长期实习 · 随时到岗<br>接受异地线下实习</p><p class="city-list">北京 / 上海 / 杭州<br>深圳 / 成都</p><span class="email-display rail-email" aria-label="邮箱：${config.email}"><svg class="gmail-icon" viewBox="0 0 24 18" aria-hidden="true"><path fill="#4285f4" d="M0 4v12a2 2 0 0 0 2 2h3V7Z"/><path fill="#34a853" d="M19 7v11h3a2 2 0 0 0 2-2V4Z"/><path fill="#ea4335" d="M5 3v4l7 5 7-5V3l-7 5Z"/><path fill="#c5221f" d="M0 4V2a2 2 0 0 1 3-1.7L5 3v4Z"/><path fill="#fbbc04" d="M19 3l2-2.7A2 2 0 0 1 24 2v2l-5 3Z"/></svg><span>${config.email}</span></span></section><section class="rail-block"><span class="eyebrow">IN PRACTICE</span><h2>从使用反馈，到工程取舍。</h2><a class="rail-case" href="${url('case','learn-agent')}"><img src="assets/learn-agent.png" width="2880" height="1824" alt="Learn-Agent 胶囊知识点和折叠卡片实际界面" loading="lazy"><strong>Learn-Agent 设计记录 <span aria-hidden="true">↗</span></strong></a><p>后台整理如何避免打断学习？卡片太长时，怎样重新组织信息？</p></section><section class="rail-block education"><span class="eyebrow">EDUCATION</span><h2>电子科技大学</h2><p>电子信息科学与技术<br>本科 · 预计 2029 年秋季毕业</p></section></aside></div>`;
}
function articles() {
  return `<header class="page-intro"><div class="eyebrow">INDEX / ${posts.length} ESSAYS</div>
    <h1>Notes & essays.</h1><p>从任务边界出发，讨论自主执行、独立验证与人的位置。</p></header>
...
function render() {
  ...
  const nav = [['home','首页'],['articles','文章'],['projects','Projects'],['about','About']];
  ...
  document.getElementById('app').innerHTML = `<div class="site-shell"><header class="site-header">
    ...
    <footer><span>© ${new Date().getFullYear()} ${escapeHtml(config.name || config.title)} · 工程与思考</span><span>Thinking in loops. Building with evidence.</span></footer></div><dialog class="contact-dialog" aria-labelledby="contact-title"><button class="dialog-close" aria-label="关闭联系窗口" autofocus>×</button><h2 id="contact-title">联系 Zaixi</h2><p>微信扫码添加好友</p><img src="assets/wechat.jpg" width="818" height="1215" alt="Zaixi 的微信好友二维码"><span class="email-display email-contact" aria-label="邮箱：${config.email}"><svg class="gmail-icon" viewBox="0 0 24 18" aria-hidden="true"><path fill="#4285f4" d="M0 4v12a2 2 0 0 0 2 2h3V7Z"/><path fill="#34a853" d="M19 7v11h3a2 2 0 0 0 2-2V4Z"/><path fill="#ea4335" d="M5 3v4l7 5 7-5V3l-7 5Z"/><path fill="#c5221f" d="M0 4V2a2 2 0 0 1 3-1.7L5 3v4Z"/><path fill="#fbbc04" d="M19 3l2-2.7A2 2 0 0 1 24 2v2l-5 3Z"/></svg><span>${config.email}</span></span></dialog>`;
  const title = page === 'post' ? posts.find(p => p.id === slug)?.title || '文章归档' :
    {home:'Agent Harness 工程师实习',articles:'文章',about:'About',projects:'Projects',case:'Learn-Agent 设计案例'}[page] || '页面未找到';
